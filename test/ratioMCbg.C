...
int ratioMCbg(string basis_, double ylow_, double yhigh_, bool logy_=0, int rebin_=1) {
  ...
  TF1* fitfunction = new TF1("fitfunction","[0]*erf([2]*(x-[1]))*(1+[3]*x)",500,2000);//erf times linear decrease at higher masses
  ...
  //TF1* fitfunction = new TF1("fitfunction","[0]*(1+[3]*TMath::Exp(-[1]*(x-[2])))/(1+TMath::Exp(-[1]*(x-[2])))*(1-[4]*x)",390,1270);//extended modified logistic function
  fitfunction -> SetParameters(0.17,300,0.01,2e-4);
  //fitfunction -> SetParLimits(1,0.0,0.1);
  fitfunction -> SetParLimits(1,100,350);
  ...
  TH1F* fiterrhist = new TH1F("fitterrhist","Histogram with fit error band",60,500,2000);
  //TH1F* fiterrhist = new TH1F("fitterrhist","Histogram with fit error band",525,260,785);
  ...
  */
  //out_can_rp -> SaveAs("Outputdata/m12_ratio_CR-SR_cuts/fitratio_TFSR_eml_m12-390-1270_Dec16-20_weights_nfsdataMay12-20.pdf");
  //out_can_rp -> SaveAs("Outputdata/m12_ratio_CR-SR_cuts/fitratio_TFSR_eml_m12-390-1270_Dec16-20_weights_nfsdataMay12-20.root");
  
  out_can_3b -> SaveAs("Outputdata/m12_ratio_CR-SR_weights/nofit_TFSR_m12-200-2000_Jan26-21_nfsdataMay12-20.pdf");
  //out_can_3b -> SaveAs("Outputdata/m12_ratio_CR-SR_cuts/fit_TFSR_eml_m12-390-1270_Dec16-20_weights_nfsdataMay12-20.root");

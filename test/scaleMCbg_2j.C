...
int scaleMCbg_2j(string var_, int rebin_ , string era_, double xlow_, double xhigh_, double ylow_, double yhigh_, bool logy_, bool cuts_) {
  ...
  cout << "(bool)cuts? [" << cuts_ << "]" << endl;

  string xtitleEnr, xtitleGeF, xtitleGes, ytitle = "Did you forget me?";
  double binning = 0.0;

  string assigndir = "rootfiles_2J1B_pTmax400300_Mar05-20/";
  string cutstr = "cuts";
  if (!cuts_) cutstr = "weights";
  string directory = ("Configs_diffBTags_allmedium/" + assigndir + cutstr + "/").c_str();

  string outfilename = (directory + "/MCbg-QCD-" + var_ + "-2017" + era_ + ".root").c_str();
  ...
  if (var == "pt_0" || (var == "pt_0_csv" || var == "pt_0_aac")){
    ...
    cout << "No known variable selected. Please select 'pt_n', 'eta_n', 'HT' or 'm12' with n=(0,1). For the quantities after cuts, please add '_aac' to the string." << endl; 
    return -1;
  }
  ...
  for (unsigned int i = 0; i < pthatbins.size(); i++){
    ...
    filesEnr[pthatbins[i]] = new TFile( (directory + "mc-bg-HT-" + pthatbins[i] + "-bEnriched-SR.root").c_str(),"READ");
    filesGeF[pthatbins[i]] = new TFile( (directory + "mc-bg-HT-" + pthatbins[i] + "-bGenFilter-SR.root").c_str(),"READ");
  ...
  /*  EnrCan -> SaveAs( ("Outputdata/" + assigndir + "mc-bg-bb-analysis-samples-" + var_ + "-2017" + era_ + "-" + cutstr + "-bEnriched.pdf").c_str() );
  EnrCan -> SaveAs( ("Outputdata/" + assigndir + "mc-bg-bb-analysis-samples-" + var_ + "-2017" + era_ + "-" + cutstr + "-bEnriched.root").c_str() );
  EnrSingleCan -> SaveAs( ("Outputdata/" + assigndir + "mc-bg-bb-analysis-samples-" + var_ + "-2017" + era_ + "-" + cutstr + "-bEnriched-single_hists.pdf").c_str() );
  EnrSingleCan -> SaveAs( ("Outputdata/" + assigndir + "mc-bg-bb-analysis-samples-" + var_ + "-2017" + era_ + "-" + cutstr + "-bEnriched-single_hists.root").c_str() );
  GeFCan -> SaveAs( ("Outputdata/" + assigndir + "mc-bg-bb-analysis-samples-" + var_ + "-2017" + era_ + "-" + cutstr + "-bGenFilter.pdf").c_str() );
  GeFCan -> SaveAs( ("Outputdata/" + assigndir + "mc-bg-bb-analysis-samples-" + var_ + "-2017" + era_ + "-" + cutstr + "-bGenFilter.root").c_str() );
  GeFSingleCan -> SaveAs( ("Outputdata/" + assigndir + "mc-bg-bb-analysis-samples-" + var_ + "-2017" + era_ + "-" + cutstr + "-bGenFilter-single_hists.pdf").c_str() );
  GeFSingleCan -> SaveAs( ("Outputdata/" + assigndir + "mc-bg-bb-analysis-samples-" + var_ + "-2017" + era_ + "-" + cutstr + "-bGenFilter-single_hists.root").c_str() );
  GesCan -> SaveAs( ("Outputdata/" + assigndir + "mc-bg-bb-analysis-samples-" + var_ + "-2017" + era_ + "-" + cutstr + "-total.pdf").c_str() );
  GesCan -> SaveAs( ("Outputdata/" + assigndir + "mc-bg-bb-analysis-samples-" + var_ + "-2017" + era_ + "-" + cutstr + "-total.root").c_str() );*/
  return 0;
}
